...
public class Spec implements ValueConstants, ToolGlobals, Serializable
{
    ...
    private void processConfigInitAndNext()
    {
        ...
        if (name.length() != 0)
        {
            ...
            this.initPredVec.addElement(new Action(def.getBody(), Context.Empty, name));
        }
        ...
        if (name.length() != 0)
        {
            ...
            this.nextPred = new Action(def.getBody(), Context.Empty, name);
        }
    }
    ...
    private void processConfigInvariants()
    {
        ...
        for (int i = 0; i < invs.size(); i++)
        {
            ...
            if (inv instanceof OpDefNode)
            {
                ...
                this.invVec.addElement(new Action(def.getBody(), Context.Empty, name));
            } else if (inv == null)
            {
        ...
        if (pred instanceof OpApplNode)
        {
            ...
            if (args.length == 0)
            {
                ...
                if (val instanceof OpDefNode)
                {
                    ...
                    {
                        this.initPredVec.addElement(new Action(Spec.addSubsts(body, subs), c, opNode.getName().toString()));
                    } else
                    {
            ...
            if (opcode == OPCODE_box)
            {
                        ...
                        && BuiltInOPs.getOpCode(((OpApplNode) boxArg).getOperator().getName()) == OPCODE_sa)
                {
                    ...
                    try
                    {
                        ...
                        SubscriptCollector collector = new SubscriptCollector(this);
                        Context c1 = c;
                        List subs1 = subs;
                        while (!subs1.isEmpty())
                        {
    ...
    private final void processConfigProps(String name, ExprNode pred, Context c, List subs)
    {
        ...
        if (pred instanceof OpApplNode)
        {
            ...
            if (opcode == OPCODE_box)
            {
                ...
                {
                    this.invVec.addElement(new Action(Spec.addSubsts(boxArg, subs), c, null));
