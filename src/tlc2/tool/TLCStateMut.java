...
public final class TLCStateMut extends TLCState implements Cloneable, Serializable {
	...
	private TLCStateMut(Value[] vals, Events events, Preconditions preconditions) { 
		this.values = vals; 
		this.events = events;
	...
	public static void init(Tool tool) {
		...
		Empty = new TLCStateMut(vals, events, pre);
	...
	public final TLCState createEmpty() {
		Value[] vals = new Value[vars.length];
		Events events = new Events();
		Preconditions pres = new Preconditions();
		return new TLCStateMut(vals, events, pres);
	}
	...
	public final TLCState copy() {
		...
		Events events = new Events(this.events);
		Preconditions pres = new Preconditions(this.preconditions);
		return new TLCStateMut(vals, events, pres);
	}
	...
	public final long fingerPrint() {
		...
		if (viewMap == null) {
			...
			if (minVals != this.values) {
				Events events = new Events();
				Preconditions pres = new Preconditions();
				state = new TLCStateMut(minVals, events, pres);
			}
	...
	@Override
	public void addEvent(OpApplNode act) {
		events.add(act);
	...
	@Override
	public void addPrecondition(ExprOrOpArgNode exprOrOpArgNode, boolean b) {
