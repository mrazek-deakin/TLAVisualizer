    ...
    implements ValueConstants, ToolGlobals, TraceApp 
{
  ...
  public final Action[] getActions() {
    if (this.actions == null) {
      ...
      if (next == null) {
        ...
        this.getActions(next.pred, next.con, null);
  ...
  private final void getActions(SemanticNode next, Context con, String parentActionName) {
    switch (next.getKind()) {
    case OpApplKind:
      {
        OpApplNode next1 = (OpApplNode)next;
        this.getActionsAppl(next1, con, parentActionName);
        return;
      }
    case LetInKind:
      {
        LetInNode next1 = (LetInNode)next;
        this.getActions(next1.getBody(), con, null);
        return;
      }
    case SubstInKind:
      {
        ...
        if (substs.length == 0) {
          ...
          Action action = new Action(next1, con, null);
    ...
    case LabelKind:
      {
        ...
        this.getActions(next1.getBody(), con, next1.toString());
        return;
      }
    default:
      {
  ...
  private final void getActionsAppl(OpApplNode next, Context con, String parentActionName) {
    ...
    if (opcode == 0) {
        ...
        if (opcode == 0) {
          try {
            ...
            if (argLevel == 0) {
              ...
              String s1 = "";
              if(opDef.getName() == null) 
            	  s1 = opDef.getName().toString();
            	  //opDef.setName(new UniqueString( opDef.toString(), 0));
              this.getActions(opDef.getBody(), con1, opDef.getName().toString());
              return;
            }
          }
      ...
      if (opcode == 0) {
        Action action = new Action(next, con, parentActionName);
        this.actionVec.addElement(action);
        return;
      }
    }
    ...
    case OPCODE_be:     // BoundedExists
      {
        ...
        try {
          ...
          while ((econ = Enum.nextElement()) != null) {
            this.getActions(args[0], econ, args[0].toString());
          }
        }
        catch (Throwable e) {
          Action action = new Action(next, con, parentActionName);
    ...
    case OPCODE_lor:      
      {
        for (int i = 0; i < args.length; i++) {
          this.getActions(args[i], con, null);
        }
        return;
      }
    default:
  ...
  private final TLCState getNextStatesAppl(OpApplNode pred, ActionItemList acts, Context c,
    ...
    if (opcode == 0) {
      ...
      //add the PrintT values to the action list - Cameron Cross 18/dec/14
      //not sure what the best way to differentiate PrintT from "now< MAX_TIME"
      //predicates, so this will have to do until i learn more
      if (pred.toString().contains("PrintT")) {
    	  s1.addEvent(pred);
      }
      ...
      if (val instanceof OpDefNode) {
        // Added by Cameron Cross 18/Dec/14, this adds the currently executed 
        // action to the state
        s1.addEvent(pred);
        OpDefNode opDef = (OpDefNode)val;
        opcode = BuiltInOPs.getOpCode(opDef.getName());
    ...
    case OPCODE_lor:      
      {
    	TLCState temp = resState.copy();
        for (int i = 0; i < alen; i++) {
          if (this.callStack != null) this.callStack.push(args[i]);       
          resState = this.getNextStates(args[i], acts, c, s0, temp.copy(), nss);
          if (this.callStack != null) this.callStack.pop();       
        }
        return resState;
      }
    ...
    case OPCODE_ite:    // IfThenElse
      {
        ...
        if (((BoolValue)guard).val) {
        	// adds the if clasuse and result to the state as a precondition
        	s1.addPrecondition(args[0], true);
          return this.getNextStates(args[1], acts, c, s0, s1, nss);
        }
        else {
        	s1.addPrecondition(args[0], false);
    ...
    case OPCODE_case:   // Case
      {
        ...
        for (int i = 0; i < alen; i++) {
          ...
          if (pairArgs[0] == null) {
            ...
            if (((BoolValue)bval).val) {
            	//adds the case clause to the state as a precondition
            	s1.addPrecondition(pairArgs[0], true);
    ...
    case OPCODE_unchanged:
      {
    	  //add UNCHANGED actions to the list of actions as they dont seem to be handled the same way
    	  // Added by Cameron Cross 18/Dec/14
    	s1.addEvent(pred);
  ...
  public final TLCStateInfo getState(long fp, TLCState s) {
    for (int i = 0; i < this.actions.length; i++) {
      ...
      for (int j = 0; j < nextStates.size(); j++) {
        ...
        long nfp = state.fingerPrint();
        if (fp == nfp) {
          return new TLCStateInfo(state, curAction.getLocationAsString());
        }
      }
    }
    return null;
